...
```text
treeman benchmark [list | branch <remote-branch> | review <pr-number> | delete | branch-results | review-results] [--runs <count>] [--warmup <count>]
```
...
`delete` takes no branch argument. Before every warmup and measured iteration, TreeMan creates the fixed disposable branch `treeman-benchmark-delete` in a temporary clone and performs normal project setup, including environment copying, branch database creation, dependency installation, and hooks. Preparation is not timed. Timing starts immediately before direct, confirmed deletion and stops when deletion returns; worktree and branch verification is also outside the timer. Setup failures abort the benchmark instead of measuring an incomplete worktree. Normal deletion safety checks remain enabled and database cleanup is included.
